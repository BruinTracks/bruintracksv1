...
def meetings_overlap(m1:Dict, m2:Dict) -> bool:
    if not set(m1['days_of_week']).intersection(m2['days_of_week']): return False
...
def build_schedule(start_y:int, start_q:str,
    ...
    name2sub = {re.sub(r"\s*\(.*\)$","",s['name']).strip().upper():s['code']
                for s in subs}

    # remove passed from required
    passed = {c for c,g in TRANSCRIPT.items() if g and meets_min_grade(g,'D-')}
    ...
    # build prereq logic via BFS+DNF
    ...
    while queue:
        ...
        for clause in clauses:
            parsed, missing = [], []
            for leaf in clause:
                if 'course' not in leaf: continue
    ...
    # fetch all sections & meeting times
    all_courses = fetch_courses(list(required))
    cid2key = {c['id']:f"{id2sub[c['subject_id']]}|{c['catalog_number']}"
    ...
    si = safe_execute(
        supa.table("section_instructors").select(
            "section_id,instructor_id"
        ).in_("section_id", [s['id'] for s in secs])
    ).data

    # map times & instructors
    mt_map, si_map = {}, {}
    for m in mt:
    ...
    instr_rows = safe_execute(
        supa.table("instructors").select("id,name")
            .in_("id", list(instr_ids))
    ).data
    ...
    def score_and_select(prefix:List[str]) -> Tuple[int,Dict[str,Dict]]:
        ...
        for course in prefix:
            ...
            for sec in sections_by_course.get(course,[]):
                ...
                sc=0
                for m in sec['times']:
                    if PREF_EARLIEST <= m['start_time'] <= PREF_LATEST: sc+=weight_map['time']
                    if PREF_EARLIEST <= m['end_time']   <= PREF_LATEST: sc+=weight_map['time']
                    if m['building'] in PREF_BUILDINGS: sc+=weight_map['building']
                    if set(m['days_of_week']).isdisjoint(PREF_NO_DAYS): sc+=weight_map['days']
                if any(i in PREF_INSTRUCTORS for i in sec['instructors']): sc+=weight_map['instructor']
                if sc>best_sc: best_sc,best_sec=sc,sec
            # discussion
            best_disc,best_dsc=None,-1
            if best_sec:
                code = best_sec['section_code'].split('-')[0]
                for dsec in sections_by_course.get(course,[]):
                    if dsec['term_id']!=idx2db[0] or dsec['is_primary']: continue
                    if not dsec['section_code'].startswith(code): continue
                    dsc=0
                    for m in dsec['times']:
                        if PREF_EARLIEST <= m['start_time'] <= PREF_LATEST: dsc+=weight_map['time']
                        if PREF_EARLIEST <= m['end_time']   <= PREF_LATEST: dsc+=weight_map['time']
                        if m['building'] in PREF_BUILDINGS: dsc+=weight_map['building']
                        if set(m['days_of_week']).isdisjoint(PREF_NO_DAYS): dsc+=weight_map['days']
                    if any(i in PREF_INSTRUCTORS for i in dsec['instructors']): dsc+=weight_map['instructor']
                    if dsc>best_dsc: best_dsc,best_disc=dsc,dsec
    ...
    for term in terms:
        ...
        base = R_rem//T_left; extra=R_rem%T_left
        ...
        if term==terms[0]:
            ...
            scored = [(sc,sel,p) for p in prefixes for sc,sel in [score_and_select(p)]]
            valid=[]
                        ...
                        for j in range(i+1,len(lec_list)):
                            for m1 in lec_list[i]['times']:
                                for m2 in lec_list[j]['times']:
                                    if meetings_overlap(m1,m2): conflict=True
                # secondary
                    ...
                    for lec in lec_list:
                        for d in dis_list:
                            for m1 in lec['times']:
                                for m2 in d['times']:
                                    if meetings_overlap(m1,m2): conflict=True
                if not conflict: valid.append((sc,sel,p))
            choices = valid or scored
            best_sc,best_sel,best_pref = max(choices, key=lambda x:x[0])
            schedule[term]=best_sel; take=best_pref
        else:
            take=avail[:target]; schedule[term]=take
        for c in take:
            for nxt in adj.get(c,[]): indegree[nxt]-=1
            remaining.discard(c)
        R_rem=len(remaining); T_left-=1

    # pad/trim
    for term in terms:
        ent=schedule[term]
        if isinstance(ent,list):
            while len(ent)<LEAST_COURSES_PER_TERM: ent.append(FILLER_COURSE)
        else:
            keys=list(ent.keys())
            while len(keys)<LEAST_COURSES_PER_TERM:
                keys.append(FILLER_COURSE); ent[FILLER_COURSE]={'lecture':None,'discussion':None}
            for extra in keys[MAX_COURSES_PER_TERM:]: ent.pop(extra,None)
            schedule[term]={k:ent[k] for k in keys[:MAX_COURSES_PER_TERM]}

    # finalize note
    scheduled_all=set()
    for ent in schedule.values():
        if isinstance(ent,dict): scheduled_all|=set(ent.keys())
        else: scheduled_all|=set(ent)
    unscheduled=set(COURSES_TO_SCHEDULE)-scheduled_all
    # remove passed from note
    unscheduled -= passed
    note=None
    if unscheduled: note="Unable to schedule: " + ", ".join(sorted(unscheduled))
    ...
    for term, ent in schedule.items():
        if isinstance(ent,dict):
            ...
            for course, info in ent.items():
                ...
                term_d[course]={'lecture':clean(info.get('lecture')),
